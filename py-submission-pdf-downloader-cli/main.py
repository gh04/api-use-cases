#!/usr/bin/env python3
"""JotForm Submission PDF Downloader — CLI.

Downloads a PDF for every submission on selected forms using the
undocumented (but JotForm-support-confirmed) pdf-converter endpoint.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download a PDF for every submission on your JotForm forms.",
    )
    parser.add_argument(
        "--api-key",
    ...
    parser.add_argument(
        "--output-dir",
        default="./pdf-downloads",
        help="Base directory for downloaded PDFs (default: ./pdf-downloads).",
    )
    parser.add_argument(
        "--workers",
        type=int,
...
def _pdf_url(base_url: str, form_id: str, submission_id: str, api_key: str) -> str:
    """Build the PDF-converter download URL for a single submission."""
    # Strip /v1 suffix to get the bare API host
    api_host = base_url.rsplit("/v1", 1)[0]
    return (
        f"{api_host}/pdf-converter/{form_id}/fill-pdf"
        f"?download=1&submissionID={submission_id}&apikey={api_key}"
    )
...
def build_pdf_list(
    ...
    """Build a download list of one PDF per submission.

    Filename format:  lastname_firstname_submissionid.pdf
    If no name field:  submissionid.pdf
    """
    files: list[dict] = []
    for sub in submissions:
        ...
        filename = "_".join(prefix_parts) + ".pdf"

        url = _pdf_url(base_url, form_id, sub_id, api_key)
...
def main() -> None:
    ...
    console.print("[bold]JotForm Submission PDF Downloader[/bold]\n")
    ...
    # 3. Download PDFs from each selected form
    ...
    for form in selected:
        ...
        if not submissions:
            console.print("[yellow]No submissions found for this form.[/yellow]")
            continue

        files = build_pdf_list(submissions, form_id, api_key, args.base_url)
        console.print(
            f"Found [bold]{len(files)}[/bold] submission(s) to download as PDF"
        )
        ...
        if result["failed"]:
            console.print(f"[red]{len(result['failed'])} PDF(s) failed:[/red]")
            for f in result["failed"]:
                console.print(f"  [red]- {f['name']}: {f['error']}[/red]")

        console.print(
            f"[green]{len(result['succeeded'])} PDF(s) downloaded to "
